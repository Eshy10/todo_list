...
const showProject = () => {
    ...
    for (let i = 0; i < note.project.length; i += 1){
       projectContainer.innerHTML += `<h6 class="text-secondary mb-3 interactive"><i class="fas fa-users-cog text-warning"></i>${note.project[i]}</h6>`
    }
    form.hideForms()
}

export default { show, showAll, showProject }